...
contract NudgeCampaignReallocationTest is Test {
  ...
  function test_HandleReallocation_Success() public {
    ...
    (uint256 expectedUserRewards, uint256 expectedFees) = campaign.calculateUserRewardsAndFees(expectedRewardAmount);

    vm.expectEmit(true, true, true, true);
    emit IBaseNudgeCampaign.NewParticipation(1, alice, expectedPID, toAmount, expectedUserRewards, expectedFees, data);
  ...
  function test_HandleReallocation_WithNativeToken_Success() public {
    ...
    vm.expectEmit(true, true, true, true);
    emit IBaseNudgeCampaign.NewParticipation(2, alice, expectedPID, toAmount, expectedUserRewards, expectedFees, data);
  ...
  function test_RevertHandleReallocation_InvalidToken() public {
    ...
    vm.prank(swapCaller);
    vm.expectRevert(abi.encodeWithSelector(IBaseNudgeCampaign.InvalidToTokenReceived.selector, address(invalidToken)));
